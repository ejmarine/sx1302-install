...
if [ "$pie34" == "y" ]; then
    ...
    if [ "$pie5" == "y" ]; then
        ...
        mv sx1302_hal_rpi5-master ~/sx1302_hal
    else
...
cp install_svc.sh ~/sx1302_hal/

read -p "Apply fix for Pi3B? (y/n)" fix

if [ "$fix" == "y" ]; then
    cp reset_lgw.sh ~/sx1302_hal/tools/reset_lgw.sh
fi
...
if [ "$add_conf" == "y" ]; then
    cp global_conf.json ~/sx1302_hal/packet_forwarder/
fi
...
echo "Finally, run 'sudo bash install_svc.sh' located in ~/sx1302_hal/ to run it as a service."
